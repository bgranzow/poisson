...
static void check_elem_indices(goal::Disc* d) {
  ...
  for (int eq = 0; eq < num_eqs; ++eq)
    goal::print("gid: %lu", d->get_gid(elem, n, eq));
  std::vector<goal::GO> gids;
  d->get_gids(elem, gids);
  for (int i = 0; i < num_dofs; ++i)
    goal::print("gids[%d]: %lu", i, gids[i]);
}
...
static void check_node_indices(goal::Disc* d) {
  ...
  for (int eq = 0; eq < num_eqs; ++eq)
    goal::print("node id: %lu", d->get_gid(nodes[0], eq));
}

static void check_nested(goal::Disc* d) {
  d->build_data();
  check_sets(d);
  check_tpetra_objs(d);
...
int main(int argc, char** argv) {
  ...
  goal::print(" > check full");
  auto n1 = goal::create_nested(d, goal::FULL);
  test::check_nested(n1);
  goal::destroy_nested(n1);
  goal::print(" > check long");
  auto n2 = goal::create_nested(d, goal::LONG);
  test::check_nested(n2);
